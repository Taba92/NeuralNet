...
-export([one_hot_encoding/1,label_encoding/1,decode/2,encode/2,most_likely/1,softmax/1]).
...
one_hot_encoding(Targets) when is_list(Targets) ->
	one_hot_encoding(Targets, length(Targets), 1, []).
one_hot_encoding([], _, _, Acc) -> Acc;
one_hot_encoding([H | T], Number, Pos, Acc) ->
	case lists:member(H, T) of
		true -> one_hot_encoding(T, Number, Pos, Acc);
		false->
			Encode = lists:duplicate(Pos - 1 , 0) ++ [1] ++ lists:duplicate(Number - Pos, 0),
			one_hot_encoding(T, Number, Pos + 1, Acc ++ [{H, Encode}])
	end.	

label_encoding(Targets) when is_list(Targets) ->
	label_encoding(lists:sort(Targets), length(Targets), 0, []).
label_encoding([], _, _, Acc) -> Acc;
label_encoding([H | T], Len, Encode, Acc) ->
	case lists:member(H, T) of
		true -> label_encoding(T, Len, Encode, Acc);
		false -> label_encoding(T, Len, Encode + 1, Acc ++ [{H, Encode}])
	end.
