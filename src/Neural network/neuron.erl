...
init(GenoType)when is_record(GenoType,neuron_classic_phenotype)->
	#neuron_classic_phenotype{id=Id}=GenoType,
	gen_server:start_link({local,Id},?MODULE,[GenoType],[]);
init([GenoType])->
	#neuron_classic_phenotype{roinsWeights=Ro}=GenoType,
	N=length(Ro),
...
handle_call(backup_weights,_,State)->
	#state{genotype=GenoType}=State,
	#neuron_classic_phenotype{bias=Bias,faninsWeights=Weights,roinsWeights=RoWeights}=GenoType,
...
handle_call(restore_weights,_,State)->
	...
	NewGenotype=GenoType#neuron_classic_phenotype{bias=OldBias,faninsWeights=OldWeights,roinsWeights=OldRoWeights},
	{reply,ok,State#state{genotype=NewGenotype}};
handle_call({perturb_weights,Prob,StepW},_,State)->
	#state{genotype=GenoType}=State,
	#neuron_classic_phenotype{bias=Bias,faninsWeights=Ins,roinsWeights=RoIns}=GenoType,
	NewState=case ?PROB(Prob) of
				true->
					...
					NewGenoType=GenoType#neuron_classic_phenotype{bias=NewBias,faninsWeights=NewFanIns,roinsWeights=NewRoIns},
...
handle_cast({ElType,FromLayer,IdFrom,FwdType,Signal},State)when ElType==sensor;ElType==neuron->
	...
	#neuron_classic_phenotype{id=Id,layer=Layer,af=Af,bias=Bias,faninsWeights=Ins,fanouts=Outs,roinsWeights=RoIns,roouts=RoOuts}=GenoType,
...
learn(GenoType,Output,Recv,RoRecv)->%%inizia ad avere segnali ricorsivi in entrata
	...
	GenoType#neuron_classic_phenotype{faninsWeights=NewIns,roinsWeights=NewRoIns}.