...
-export([clone_element/1, clone_synapse/2, mutate/3,mutate_weights/2,mutate_plasticity/2,mutate_bias/2,mutate_af/2,add_neuro_link/2,add_sensor_link/2,
...
clone_synapse(SynapseGenotype, MappingIds) when is_record(SynapseGenotype, synapses),is_list(MappingIds) ->
	...
	{{NewIdFrom, NewIdTo}, SynapseGenotype#synapses{id_from = NewIdFrom, id_to = NewIdTo}}.
...
mutate(Genotype, NMutation, Constraint)->
	...
	lists:foldl(fun(MutationFun, {MutatedGenotype, MutationsApplied}) -> 
					{NewMutatedGenotype, Mutation} = ?MODULE:MutationFun(MutatedGenotype, SpecificsConstraint),
					{NewMutatedGenotype, MutationsApplied ++ [Mutation]}
				end, 
				{Genotype, []}, 
				Mutators
			).
...
mutate_plasticity(Genotype, Constraint) ->
...
mutate_bias(Genotype, _) ->
	%1) Select random neuron
	NeuronId = ?RANDCHOOSE(genotype:get_neuron_ids(Genotype)),
	NeuronGenotype = genotype:get_element_by_id(Genotype, NeuronId),
	%2) Perturb the bias
	NewBias = ?NN_SERVICE_MODULE:perturbate(NeuronGenotype#neuron_classic_genotype.bias),
...
add_neuro_link(Genotype, Constraint) ->%link tra neuroni,o su se stesso oppure con un altro neurone in qualsiasi strato.
	{plast, Plasts} = lists:keyfind(plast, 1, Constraint),
	...
	{NeuronFromGenotype, NeuronToGenotype} = {genotype:get_element_by_id(Genotype, NeuronIdFrom), genotype:get_element_by_id(Genotype, NeuronIdTo)},
	%2) Add the synapse between the two neurons if not already exist
	case genotype:get_synapses(Genotype, NeuronIdFrom, NeuronIdTo) of
		false ->
...
add_sensor_link(Genotype, Constraint) ->
	{plast,Plasts} = lists:keyfind(plast, 1, Constraint),
	%1) Select a random sensor and a random neuron
	{SensorId, NeuronId} = {?RANDCHOOSE(genotype:get_sensors_ids(Genotype)), ?RANDCHOOSE(genotype:get_neuron_ids(Genotype))},
	{SensorGenotype, NeuronGenotype} = {genotype:get_element_by_id(Genotype, SensorId), genotype:get_element_by_id(Genotype, NeuronId)},
...
add_neuron(Genotype, Constraint)->
	{plast, Plasts} = lists:keyfind(plast, 1, Constraint),
	{af, Afs} = lists:keyfind(af, 1, Constraint),
		...
		_ -> 
			...
			case LayerWhereInsertTheNeuron of
				...
				H -> 
					...
					SensorGenotype = genotype:get_element_by_id(Genotype, SensorId),
...
add_layer_neuron(Genotype, Constraint) ->%add a new layer between two other layers
	{plast, Plasts} = lists:keyfind(plast, 1, Constraint),
	{af, Afs} = lists:keyfind(af, 1, Constraint),
	PlasticityModulator = plasticity:random_plasticity(Plasts),
	ActivationFunction = ?RANDCHOOSE(Afs),
		...
		N when N > 1 ->
			...
			case ?RANDCHOOSE([0,1]) of
		 		...
		 		0 ->
					case LayerReference of
						...
						_ ->
							...
							LastLayerBeforeReferenceLayer = lists:last(InfLayers),
							...
							PredicateInf = fun(Element) -> is_record(Element, neuron_classic_genotype) andalso Element#neuron_classic_genotype.layer == LastLayerBeforeReferenceLayer end,
				...
				1 -> 
					...
					LayerNextReferenceLayer = hd(SupLayers),
					%2) Create the new index layer
